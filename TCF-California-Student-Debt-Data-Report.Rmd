...
agg60$California <- ifelse(agg60$State.Abbreviation=="CA", 1, 0)

f39A1 <- agg60 %>% arrange(`Bachelor's degree`)
g39A1 <- ggplot(data=agg60, mapping=aes(x=`State.Abbreviation`, y=`Bachelor's degree`, fill=California, text=`For Tooltip`)) + geom_bar(stat="identity") + labs(x="State", y="Share of B.A. holders earning $60,000 or more") + scale_fill_manual(values = rev(flagCols2[4:5])) + theme(legend.position='none', text=element_text(family="Optima"), axis.text = element_text(size = 7)) 

ggplotly(g39A1, width=900, height=300, tooltip="text")
```

```{r }
f39A2 <- agg60 %>% arrange(`High school diploma or equivalent`)
g39A2 <- ggplot(data=agg60, mapping=aes(x=`State.Abbreviation`, y=`High school diploma or equivalent`, fill=California, text=`For Tooltip`)) + geom_bar(stat="identity") + labs(x="State", y="Share of H.S. diploma holders earning $60,000 or more") + scale_fill_manual(values = rev(flagCols2[4:5])) + theme(legend.position='none', text=element_text(family="Optima"), axis.text = element_text(size = 7)) 

ggplotly(g39A2, width=900, height=300, tooltip="text")
```
