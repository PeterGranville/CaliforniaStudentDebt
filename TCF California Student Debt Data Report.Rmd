...
# For all workers:
PWGTP.st.ed <- aggregate(data=pusm1519, PWGTP ~ State + Education, FUN=sum)
names(PWGTP.st.ed)[3] <- "Weight in group"
pusm1519 <- left_join(x=pusm1519, y=PWGTP.st.ed, by=c("State", "Education"))
pusm1519 <- pusm1519 %>% mutate(`Adjusted WAGP` = `WAGP` * (`PWGTP` / `Weight in group`))
agg4 <- aggregate(data=pusm1519, `Adjusted WAGP` ~ State + Education, FUN=sum)
...
agg5 <- aggregate(data=pusm1519S1, `Adjusted WAGP` ~ Education + Race + Sex, FUN=sum)
agg5 <- reshape2::dcast(data=agg5, Race + Sex ~ Education, value.var="Adjusted WAGP")
agg5 <- agg5 %>% mutate(`Difference: BA over HS` = `Bachelor's degree` - `High school diploma or equivalent`)
agg5 <- agg5 %>% mutate(`Ratio: BA over HS` = `Bachelor's degree` / `High school diploma or equivalent`)
agg5 <- agg5 %>% mutate(`Difference: GD over BA` =  `Graduate degree` - `Bachelor's degree`)
agg5 <- agg5 %>% mutate(`Ratio: GD over BA` = `Graduate degree` / `Bachelor's degree`)
...
agg6 <- aggregate(data=pusm1519S2, `Adjusted WAGP` ~ Education + Race, FUN=sum)
agg6 <- reshape2::dcast(data=agg6, Race ~ Education, value.var="Adjusted WAGP")
