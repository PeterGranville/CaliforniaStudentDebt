...
npsas1$`For Tooltip` <- paste(
  ...
  "Group: ", npsas1$Group, '\n', 
  "Average loan: ", dollar(npsas1$`Average Loan`, accuracy=1), 
  sep="")
...
```{r figure6B}
g6B <- ggplot(data=npsas5.HSP, mapping=aes(y=Group, x=`Total federal loans`, fill=Group)) + geom_bar(stat="identity", position="dodge") + labs(title="Average federal student loans by Hispanic ethinicity: U.S. vs CA", x="Average federal student loans") + facet_grid(State ~ .) + scale_x_continuous(labels=scales::dollar_format()) + scale_fill_brewer(palette="Set3") + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g6B, width=800, height=500)
...
```{r figure7}
g7 <- ggplot(data=tblOwnDebt, mapping=aes(y=Race, x=`Share with debt`, fill=State)) + geom_bar(stat="identity", position="dodge", width=0.7) + scale_x_continuous(labels=scales::percent_format(accuracy=1)) + labs(y="", title="") + theme(text=element_text(family="Optima")) + scale_fill_brewer(palette="Dark2")

ggplotly(g7, width=800, height=350)
...
```{r figure8A}
g8A <- ggplot(data=tblOwnDebtSL, mapping=aes(y=Race, x=`Share of borrowers whose debt includes a student loan`, fill=State)) + geom_bar(stat="identity", position="dodge", width=0.7) + scale_x_continuous(labels=scales::percent_format(accuracy=1), limits=c(0, 1)) + labs(y="", title="") + theme(text=element_text(family="Optima")) + scale_fill_brewer(palette="Dark2")

ggplotly(g8A, width=800, height=350)
...
ggplotly(g8B, width=800, height=350)
...
```{r figure8C}
g8C <- ggplot(data=tblOwnDebtCC, mapping=aes(y=Race, x=`Share of borrowers whose debt includes credit card debt`, fill=State)) + geom_bar(stat="identity", position="dodge", width=0.7) + scale_x_continuous(labels=scales::percent_format(accuracy=1), limits=c(0, 1)) + labs(y="", title="") + theme(text=element_text(family="Optima")) + scale_fill_brewer(palette="Dark2") 

ggplotly(g8C, width=800, height=350)
...
The survey years included for DiffSchool1 are
```{r DiffSchool1.SY, results = 'asis'}
allYears <- rev(unique(shed.DiffSchool1$SurveyYear))
