...
We use the data on loan disbursements, not loan originations.
...
g1 <- ggplot(data=table4.1, mapping=aes(x=State, y=`Average Loan`, fill=State, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.4) + labs(title="Average annual loans by type and sector: California vs. Rest of U.S.") + scale_y_continuous(labels=scales::dollar_format()) + facet_grid(Control ~ variable) + scale_fill_manual(values = stateCols) + theme(text=element_text(family="Optima")) 
...
We use the data on loan disbursements, not loan originations. These averages do not include students who receive no loans. Only four-year institutions are included. 
...
g2A <- ggplot(data=npsas1, mapping=aes(x=State, y=`Average Loan`, fill=State, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.4) + labs(title="Average annual undergrad. loans at public 4-years by group: CA vs U.S.") + facet_grid(variable ~ Group) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g2A, tooltip="text", width=800, height=400)
```
##### Figure 2: Race and Gender (U.S. only)
```{r npsas1.1}
npsas1.1 <- read.csv("Powerstats ygpebv.csv", skip=12, nrow=14, header=FALSE)
names(npsas1.1) <- c("Group", "Federal loans (excl. PLUS)", "Parent PLUS")

npsas1.1$State <- rep("U.S.", nrow(npsas1.1))
...
npsas1.1$Group <- factor(npsas1.1$Group, levels = c("White", "Latino/a", "Black", "Asian", "Pacific Islander", "Native American", "2+ races"))
...
npsas1.1$`For Tooltip` <- paste(
  ...
  "Loan type: ", npsas1.1$variable, '\n',
  "Group: ", npsas1.1$Group, '\n', 
  "Gender: ", npsas1.1$Gender, '\n', 
  "Average annual loans: ", dollar(npsas1.1$`Average Loan`, accuracy=1), 
  sep="")

g2B <- ggplot(data=npsas1.1, mapping=aes(x=Gender, y=`Average Loan`, fill=Gender, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.4) + labs(title="Average annual undergrad. loans at public 4-years by group and gender (U.S. only)") + facet_grid(variable ~ Group) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = flagCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g2B, tooltip="text", width=800, height=400)
```
##### Data source and notes

**Data source:** National Postsecondary Student Aid Study  Administrative Collection, Undergraduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval number *lounfy* for "Figure 2: Race (CA and U.S.)" and *ygpebv* for "Figure 2: Race and Gender (U.S. only)."

**Timeframe reflected:** Figures in this graph reflect students enrolled in the 2017-18 academic year. 

 Only reflects undergraduate public 4-year institutions.  

Note: Insufficient sample size for Native American / Alaskan Native population in "Figure 2: Race (CA and U.S.). Averages include those who took out no loans. 

***

#### {-}

**Say exactly which columns are flagged by NPSAS for low sample size...**
...
g3 <- ggplot(data=npsas2, mapping=aes(x=State, y=`Total federal loans`, fill=State, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.6) + labs(title="Average annual fed. student loans for graduate students, by group: CA vs U.S.", y="Average Loan") + facet_grid(. ~ Group) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g3, height=350, tooltip="text")
```
##### Figure 3: Race and Gender
```{r npsas2.1}
npsas2.1.US <- read.csv("Powerstats uklsko.csv", skip=12, nrow=14, header=FALSE)
npsas2.1.CA <- read.csv("Powerstats uklsko.csv", skip=108, nrow=14, header=FALSE)

names(npsas2.1.US) <- c("Group", "Total federal loans")
names(npsas2.1.CA) <- c("Group", "Total federal loans")

npsas2.1.US$State <- rep("U.S.", nrow(npsas2.1.US))
npsas2.1.CA$State <- rep("CA", nrow(npsas2.1.CA))
npsas2.1 <- rbind(npsas2.1.US, npsas2.1.CA)
...
npsas2.1$Group <- factor(npsas2.1$Group, levels = c("White", "Latino/a", "Black", "Asian"))

npsas2.1$`Total federal loans` <- gsub(pattern="!", x=npsas2.1$`Total federal loans`, replacement="")
npsas2.1$`Total federal loans` <- gsub(pattern=",", x=npsas2.1$`Total federal loans`, replacement="")
npsas2.1$`Total federal loans` <- gsub(pattern=" ", x=npsas2.1$`Total federal loans`, replacement="")
npsas2.1$`Total federal loans` <- as.numeric(npsas2.1$`Total federal loans`)
```
...
npsas2.1$`For Tooltip` <- paste(
  ...
  "Average annual loans: ", dollar(npsas2.1$`Total federal loans`, accuracy=1), 
  sep="")

g3B <- ggplot(data=npsas2.1, mapping=aes(x=State, y=`Total federal loans`, fill=State, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.6) + labs(title="Average annual fed. student loans for graduate students, by group and gender: CA vs U.S.") + facet_grid(Gender ~ Group) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g3B, tooltip="text", width=800, height=400)
```
##### Data source and notes 

**Data source:** National Postsecondary Student Aid Study, Undergraduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval number *psiqll* for "Figure 3: Race" and *uklsko* for "Figure 3: Race and Gender."
...
npsas3.US.certificate <- read.csv("Powerstats_vcbaev.csv", skip=134, nrow=7, header=FALSE)
npsas3.US.phd <- read.csv("Powerstats_vcbaev.csv", skip=195, nrow=7, header=FALSE)
npsas3.US.professional <- read.csv("Powerstats_vcbaev.csv", skip=256, nrow=7, header=FALSE)

names(npsas3.US.masters) <- c("Group", "Total federal loans")
names(npsas3.US.certificate) <- c("Group", "Total federal loans")
names(npsas3.US.phd) <- c("Group", "Total federal loans")
...
npsas3.US.certificate$Level <- rep("Post-bac certificates", nrow(npsas3.US.certificate))
npsas3.US.phd$Level <- rep("Research doctorates", nrow(npsas3.US.phd))
npsas3.US.professional$Level <- rep("Professional doctorates", nrow(npsas3.US.professional))

npsas3 <- rbind(npsas3.US.masters, 
                npsas3.US.certificate, 
                npsas3.US.phd, 
...
npsas3$Group <- factor(npsas3$Group, levels = c("White", "Latino/a", "Black", "Asian", "Pacific Islander", "2+ races"))
...
ggplotly(g4A, width=800, height=450, tooltip="text")
```
##### Figure 4: Race and Gender
```{r npsas3.1}
npsas3.1.US.masters <- read.csv("Powerstats pasqwo.csv", skip=108, nrow=14, header=FALSE)
npsas3.1.US.phd <- read.csv("Powerstats pasqwo.csv", skip=300, nrow=14, header=FALSE)
npsas3.1.US.professional <- read.csv("Powerstats pasqwo.csv", skip=396, nrow=14, header=FALSE)

names(npsas3.1.US.masters) <- c("Group", "Total federal loans")
names(npsas3.1.US.phd) <- c("Group", "Total federal loans")
...
npsas3.1.US.phd$Level <- rep("Research doctorates", nrow(npsas3.1.US.phd))
...
g4B <- ggplot(data=npsas3.1, mapping=aes(x=Group, y=`Total federal loans`, fill=Group, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.6) + labs(title="Average annual fed. loans for graduate students by group, gender, and level: U.S. only", caption="Groups not shown lacked sufficient samples.") + facet_grid(Gender ~ Level) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g4B, width=800, height=450, tooltip="text")
```
##### Data source and notes

**Data source:** National Postsecondary Student Aid Study, Graduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval number *vcbaev* for "Figure 4: Race" and table retrieval number *pasqwo* for "Figure 4: Race and Gender."
...
g5A <- ggplot(data=npsas4.1, mapping=aes(x=State, y=`Average cumulative federal loans`, fill=State, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.6) + facet_grid(. ~ Group) + scale_y_continuous(label=scales::dollar) + labs(x="State", title="Average cumulative federal loans: Graduating undergraduates", y="Average cumulative loans") + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g5A, width=800, height=350, tooltip="text")
```
##### Figure 5: Race and Gender (U.S. only)
```{r npsas4.2, warning=FALSE}
npsas4.2 <- read.csv("PowerStats fvvhig.csv", skip=12, nrow=14, header=FALSE)
names(npsas4.2) <- c("Group", "Average cumulative federal loans")
npsas4.2$State <- rep("U.S.", nrow(npsas4.2))
npsas4.2$Gender <- rep(NA, nrow(npsas4.2))

npsas4.2$Gender[npsas4.2$Group=="Black or African American male"] <- "Male"
npsas4.2$Gender[npsas4.2$Group=="Black or African American female"] <- "Female"
npsas4.2$Group[npsas4.2$Group=="Black or African American male"] <- "Black"
npsas4.2$Group[npsas4.2$Group=="Black or African American female"] <- "Black"

npsas4.2$Gender[npsas4.2$Group=="Hispanic or Latino male"] <- "Male"
npsas4.2$Gender[npsas4.2$Group=="Hispanic or Latino female"] <- "Female"
npsas4.2$Group[npsas4.2$Group=="Hispanic or Latino male"] <- "Latino/a"
npsas4.2$Group[npsas4.2$Group=="Hispanic or Latino female"] <- "Latino/a"

npsas4.2$Gender[npsas4.2$Group=="Asian male"] <- "Male"
npsas4.2$Gender[npsas4.2$Group=="Asian female"] <- "Female"
npsas4.2$Group[npsas4.2$Group=="Asian male"] <- "Asian"
npsas4.2$Group[npsas4.2$Group=="Asian female"] <- "Asian"

npsas4.2$Gender[npsas4.2$Group=="Pacific Islander/Hawaiian male"] <- "Male"
npsas4.2$Gender[npsas4.2$Group=="Pacific Islander/Hawaiian female"] <- "Female"
npsas4.2$Group[npsas4.2$Group=="Pacific Islander/Hawaiian male"] <- "Pacific Islander"
npsas4.2$Group[npsas4.2$Group=="Pacific Islander/Hawaiian female"] <- "Pacific Islander"

npsas4.2$Gender[npsas4.2$Group=="White male"] <- "Male"
npsas4.2$Gender[npsas4.2$Group=="White female"] <- "Female"
npsas4.2$Group[npsas4.2$Group=="White male"] <- "White"
npsas4.2$Group[npsas4.2$Group=="White female"] <- "White"

npsas4.2$Gender[npsas4.2$Group=="American Indian or Alaska Native male"] <- "Male"
npsas4.2$Gender[npsas4.2$Group=="American Indian or Alaska Native female"] <- "Female"
npsas4.2$Group[npsas4.2$Group=="American Indian or Alaska Native male"] <- "Native American"
npsas4.2$Group[npsas4.2$Group=="American Indian or Alaska Native female"] <- "Native American"

npsas4.2$Gender[npsas4.2$Group=="Male of more than one race"] <- "Male"
npsas4.2$Gender[npsas4.2$Group=="Female of more than one race"] <- "Female"
npsas4.2$Group[npsas4.2$Group=="Male of more than one race"] <- "2+ races"
npsas4.2$Group[npsas4.2$Group=="Female of more than one race"] <- "2+ races"

npsas4.2$Gender <- as.factor(npsas4.2$Gender)
npsas4.2$Gender <- factor(npsas4.2$Gender, levels=c("Male", "Female"))
```

```{r figure5B}
npsas4.2$`For Tooltip` <- paste(
  "State: ", npsas4.2$State, '\n',
  "Group: ", npsas4.2$Group, '\n',
  "Gender: ", npsas4.2$Gender, '\n',
  "Average cumulative federal loans: ", dollar(npsas4.2$`Average cumulative federal loans`, accuracy=1),
  sep=""
)

g5B <- ggplot(data=npsas4.2, mapping=aes(x=Gender, y=`Average cumulative federal loans`, fill=Gender, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.6) + facet_grid(. ~ Group) + scale_y_continuous(label=scales::dollar) + labs(x="Gender", title="Average cumulative federal loans: Graduating undergraduates (U.S. only)", y="Average cumulative loans") + scale_fill_manual(values = flagCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g5B, width=800, height=350, tooltip="text")
```
##### Data source and notes

**Data source:** National Postsecondary Student Aid Study, Undergraduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval numbers *upgkeu* and *vsqgcr* for "Figure 5: Race" and table retrieval number *fvvhig* for "Figure 5: Race and Gender (U.S. only)".
...
npsas5.HSP$Group <- factor(npsas5.HSP$Group, levels=rev(c(
  "Mexican or Chicano descent", 
  "Cuban descent", 
  "Puerto Rican descent",
  "Other Hispanic or Latino descent", 
  "Mixed Hispanic origin"
)))

npsas5.ASN$Group <- as.factor(npsas5.ASN$Group)
npsas5.ASN$Group <- factor(npsas5.ASN$Group, levels=rev(c(
  "Chinese", 
  "Indian", 
  "Japanese", 
  "Korean", 
...
**Data source:** National Postsecondary Student Aid Study, Undergraduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval numbers *tpsawz* (U.S.) and *jejvld* (CA).
...
Due to smaller sample size, interpret results with caution.
...
g29A <- ggplot(data=tblThreeMonSave, mapping=aes(y=Race, x=`Share`, fill=Group, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.7) + scale_x_continuous(labels=scales::percent_format(accuracy=1), limits=c(0, 1)) + labs(y="", title="", x="Share with sufficient funds set aside", fill="") + theme(text=element_text(family="Optima")) + scale_fill_manual(values = stateCols)
...
g29B <- ggplot(data=tblThreeMonCover, mapping=aes(y=Race, x=`Share`, fill=Group, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.7) + scale_x_continuous(labels=scales::percent_format(accuracy=1), limits=c(0, 1)) + labs(y="", title="", x="Share who could cover three months' expenses", fill="") + theme(text=element_text(family="Optima")) + scale_fill_manual(values = stateCols)
...
# I have checked and verified that the survey years for both verisons of Figure 29 match.
```
