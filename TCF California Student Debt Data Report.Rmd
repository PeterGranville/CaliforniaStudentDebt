...
##### Table 3
```{r kable3.1}
kable(table3.1, caption="Share of loan dollars disbursed in 2021-22 by loan type, California vs. Rest of U.S.") %>% kable_paper("hover", full_width = F)
```
##### Table 3: All states
```{r kable3.2}
kable(table3.2, caption="Share of loan dollars disbursed in 2021-122 by loan type") %>% kable_paper("hover", full_width = F)
```
...
*Is the SHED sample representative of California households?*

Because the SHED sample can get small for certain sub-groups that are of interest to our analysis, sample bias can become a concern. 
...
We can start by determining the 
...
# We're primarily interested in 37, so that's what we'll focus on. 
# Figure 37 
...
# Black women in CA vs Overall CA 

# Figure 5: Share borrowing 
# Black in CA
# Latino in CA
# Pacific Islander in CA
# Asian in CA 
# White in CA 
# Overall CA
...
derivePLUStotals <- function(loansyear, loanfile, nskip, colnames) {
  loans <- fread(loanfile, header=TRUE, skip=nskip)
  names(loans) <- colnames
  
  if(colnames==colnames1){
    loans <- loans %>% select(
      `OPEID`,
      `State`, 
      `SUBSIDIZED: $ of Disbursements`, 
  ...
  loans <- loans %>% mutate(`PARENT PLUS share of total` = `PARENT PLUS: $ of Disbursements` / `Total $ of Disbursements`)
  loans <- loans %>% mutate(`GRAD PLUS share of total` = `GRAD PLUS: $ of Disbursements` / `Total $ of Disbursements`)
  loans$Year <- rep(loansyear, nrow(loans))
  return(results)
}

plusOverTime <- rbind(
  derivePLUStotals(loansyear = 2006, loanfile = loanfile06, nskip = nskip06, colnames = colnames3),
  derivePLUStotals(loansyear = 2007, loanfile = loanfile07, nskip = nskip07, colnames = colnames3),
  derivePLUStotals(loansyear = 2008, loanfile = loanfile08, nskip = nskip08, colnames = colnames3), 
  derivePLUStotals(loansyear = 2009, loanfile = loanfile09, nskip = nskip09, colnames = colnames3), 
  derivePLUStotals(loansyear = 2010, loanfile = loanfile10, nskip = nskip10, colnames = colnames3), 
  derivePLUStotals(loansyear = 2011, loanfile = loanfile11, nskip = nskip11, colnames = colnames2), 
  derivePLUStotals(loansyear = 2012, loanfile = loanfile12, nskip = nskip12, colnames = colnames2), 
  derivePLUStotals(loansyear = 2013, loanfile = loanfile13, nskip = nskip13, colnames = colnames1), 
  derivePLUStotals(loansyear = 2014, loanfile = loanfile14, nskip = nskip14, colnames = colnames1), 
  derivePLUStotals(loansyear = 2015, loanfile = loanfile15, nskip = nskip15, colnames = colnames1), 
  derivePLUStotals(loansyear = 2016, loanfile = loanfile16, nskip = nskip16, colnames = colnames1), 
  derivePLUStotals(loansyear = 2017, loanfile = loanfile17, nskip = nskip17, colnames = colnames1), 
  derivePLUStotals(loansyear = 2018, loanfile = loanfile18, nskip = nskip18, colnames = colnames1), 
  derivePLUStotals(loansyear = 2019, loanfile = loanfile19, nskip = nskip19, colnames = colnames1), 
  derivePLUStotals(loansyear = 2020, loanfile = loanfile20, nskip = nskip20, colnames = colnames1), 
  derivePLUStotals(loansyear = 2021, loanfile = loanfile21, nskip = nskip21, colnames = colnames1), 
  derivePLUStotals(loansyear = 2022, loanfile = loanfile22, nskip = nskip22, colnames = colnames1)
)
