...
g2A <- ggplot(data=npsas1, mapping=aes(x=State, y=`Average Loan`, fill=State, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.4) + labs(title="Average annual undergraduate loans by group: CA vs U.S.") + facet_grid(. ~ Group) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g2A, tooltip="text", width=800, height=350)
```
##### Figure 2: Race and Gender 
```{r npsas1.1}
...
g2B <- ggplot(data=npsas1.1, mapping=aes(x=State, y=`Average Loan`, fill=State, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.4) + labs(title="Average annual undergraduate loans by group and gender: CA vs. U.S.") + facet_grid(Gender ~ Group) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = flagCols) + theme(legend.position="none", text=element_text(family="Optima"))
...
g3B <- ggplot(data=npsas2.1, mapping=aes(x=State, y=`Total federal loans`, fill=State, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.6) + labs(title="Average annual graduate loans by group and gender: CA vs U.S.") + facet_grid(Gender ~ Group) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))
...
#### {.tabset}
```{r npsas3}
npsas3.US.masters <- read.csv("PowerStats svwuag.csv", skip=73, nrow=7, header=FALSE)
npsas3.US.phd <- read.csv("PowerStats svwuag.csv", skip=195, nrow=7, header=FALSE)
npsas3.US.professional <- read.csv("PowerStats svwuag.csv", skip=256, nrow=7, header=FALSE)
...
npsas3 <- npsas3 %>% filter(Group %in% c("White", "Latino/a", "Black"))

npsas3$Group <- as.factor(npsas3$Group)
npsas3$Group <- factor(npsas3$Group, levels = c("White", "Latino/a", "Black"))
...
```
##### Figure 4: Race and Gender (U.S. only)
```{r npsas3.1}
npsas3.1.US.masters <- read.csv("PowerStats lhslzf.csv", skip=108, nrow=14, header=FALSE)
npsas3.1.US.phd <- read.csv("PowerStats lhslzf.csv", skip=300, nrow=14, header=FALSE)
npsas3.1.US.professional <- read.csv("PowerStats lhslzf.csv", skip=396, nrow=14, header=FALSE)
...
**Data source:** National Postsecondary Student Aid Study, Graduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval number *svwuag* for "Figure 4: Race" and table retrieval number *lhslzf* for "Figure 4: Race and Gender."
...
npsas4.CA <- read.csv("PowerStats dxwsoi", skip=73, nrow=7, header=FALSE)
...
```
##### Figure 5: Race and Gender
```{r npsas4.1, warning=FALSE}
npsas4.1.US <- read.csv("PowerStats gkhfjw.csv", skip=12, nrow=14, header=FALSE)
npsas4.1.CS <- read.csv("PowerStats gkhfjw.csv", skip=108, nrow=14, header=FALSE)

####ProgressMark####

names(npsas4.1) <- c("Group", "Average cumulative federal loans")
npsas4.1$State <- rep("U.S.", nrow(npsas4.1))
...
**Notes:** Due to limitations of NPSAS:16, the data in Figure 5 only apply to in-state students. The averages here include those with no cumulative federal loans. 
...
**Notes:** For this analysis, I filtered for 4-year colleges and U.S. citizenship. "Total federal loans" includes Parent PLUS. No other racial groups besides Hispanic and Asian have breakouts by ethnicity in NPSAS. Due to limitations of NPSAS:16, the data in Figure 6 only apply to in-state students. 
