...
tblEdAttain <- tblEdAttain %>% add_row(EdAttain="Less than high school diploma", Race="2+ Races, Non-Hispanic", State="CA", AnyStudentLoans="Yes", Weight=0)
tblEdAttain <- tblEdAttain %>% add_row(EdAttain="High school degree or GED", Race="Other, Non-Hispanic", State="CA", AnyStudentLoans="Yes", Weight=0)
...
g12A <- ggplot(data=tblEdAttain.CA.Borrow, mapping=aes(y=Race, x=`Share of borrowers`, fill=variable)) + geom_bar(stat="identity") + scale_x_continuous(labels=scales::percent_format(accuracy=1)) + labs(fill="Highest level of education", x="Share of student loan borrowers in California") + theme(text=element_text(family="Optima")) + scale_fill_brewer(palette="Spectral") 
...
g12B <- ggplot(data=tblEdAttain.US.Borrow, mapping=aes(y=Race, x=`Share of borrowers`, fill=variable)) + geom_bar(stat="identity") + scale_x_continuous(labels=scales::percent_format(accuracy=1)) + labs(fill="Highest level of education", x="Share of student loan borrowers in the rest of the U.S.") + theme(text=element_text(family="Optima")) + scale_fill_brewer(palette="Spectral") 
...
g12C <- ggplot(data=tblEdAttain.CA.Nonborrow, mapping=aes(y=Race, x=`Share of borrowers`, fill=variable)) + geom_bar(stat="identity") + scale_x_continuous(labels=scales::percent_format(accuracy=1)) + labs(fill="Highest level of education", x="Share of non-borrowers in California") + theme(text=element_text(family="Optima")) + scale_fill_brewer(palette="Spectral") 
...
", Race="2+ Races, Non-Hispanic", State="CA", AnyStudentLoans="Yes", Weight=0)
...
tblHomeOwn.CA.Borrow <- tblHomeOwn %>% filter(State=="CA") %>% filter(AnyStudentLoans=="Yes") %>% filter(Race != "2+ Races, Non-Hispanic")
...
tblHomeOwn.US.Borrow <- tblHomeOwn %>% filter(State=="Rest of U.S.") %>% filter(AnyStudentLoans=="Yes") %>% filter(Race != "2+ Races, Non-Hispanic")
...
tblHomeOwn.CA.Nonborrow <- tblHomeOwn %>% filter(State=="CA") %>% filter(AnyStudentLoans=="No") %>% filter(Race != "2+ Races, Non-Hispanic")
...
```
#### Data source and notes
Data not sufficient for "2+ Races, Non-Hispanic" group. 
***
### {-}
...
ggplotly(g14A, width=700, height=400)
```
...
ggplotly(g14B, width=700, height=400)
```
#### Data source and notes
***
### {-}
...
tblSavings <- tblSavings %>% add_row(Savings="$250,000 - $499,999", Race="2+ Races, Non-Hispanic", State="CA", AnyStudentLoans="Yes", Weight=0)
