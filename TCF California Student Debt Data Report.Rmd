...
To lesser extents, I also use data from the Integrated Postsecondary Education Data System (IPEDS) and the American Community Survey (ACS). 
...
names(npsas1.US) <- c("Group", "Average federal loans")
names(npsas1.CA) <- c("Group", "Average federal loans")
...
npsas1P$Group <- factor(npsas1P$Group, levels = c("White", "Latino/a", "Black", "Asian", "Pacific Islander", "2+ races"))

npsas1P$`Share Borrowing Loans` <- gsub(pattern="!", x=npsas1P$`Share Borrowing Loans`, replacement="")
npsas1P$`Share Borrowing Loans` <- gsub(pattern=",", x=npsas1P$`Share Borrowing Loans`, replacement="")
npsas1P$`Share Borrowing Loans` <- gsub(pattern=" ", x=npsas1P$`Share Borrowing Loans`, replacement="")
...
names(npsas1.1.US) <- c("Group", "Average federal loans")
names(npsas1.1.CA) <- c("Group", "Average federal loans")
...
npsas1.1$Group <- factor(npsas1.1$Group, levels = c("White", "Latino/a", "Black", "Asian", "2+ races"))

npsas1.1$`Average Loan` <- gsub(pattern="!", x=npsas1.1$`Average Loan`, replacement="")
npsas1.1$`Average Loan` <- gsub(pattern=",", x=npsas1.1$`Average Loan`, replacement="")
npsas1.1$`Average Loan` <- gsub(pattern=" ", x=npsas1.1$`Average Loan`, replacement="")
...
npsas1.1P <- npsas1.1P %>% filter(Group %in% c("Native American")==FALSE)
...
npsas1.1P$Group <- factor(npsas1.1P$Group, levels = c("White", "Latino/a", "Black", "Asian", "Pacific Islander", "2+ races"))

npsas1.1P$`Share Borrowing Loans` <- gsub(pattern="!", x=npsas1.1P$`Share Borrowing Loans`, replacement="")
npsas1.1P$`Share Borrowing Loans` <- gsub(pattern=",", x=npsas1.1P$`Share Borrowing Loans`, replacement="")
npsas1.1P$`Share Borrowing Loans` <- gsub(pattern=" ", x=npsas1.1P$`Share Borrowing Loans`, replacement="")
...
**Data source:** National Postsecondary Student Aid Study, Undergraduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval number *opspco* for "Figure 2: Race" and *ztmgmm* for "Figure 2: Race and Gender."

**Timeframe reflected:** Figures in this graph reflect students enrolled in the 2015-16 academic year. 

**Notes:** Due to limitations of NPSAS:16, the data in Figure 2 only apply to in-state students. The averages here do not include those who did not borrow federal loans. These measures include all federal student loans (including Parent PLUS) and do not include any private or institutional loans. 

***

#### {-}

Figure 3 shows that key differences at the graduate level. The average annual loans are significantly larger than at the undergraduate level, and California more sharply exceeds the nation in the average loan. In addition, the share of California students who borrow is much higher at the graduate level than the undergraduate level. 

#### {.tabset}
##### Figure 3: Average Graduate Loans by Race
```{r npsas2}
npsas2.US <- read.csv("Powerstats ozhexe.csv", skip=12, nrow=7, header=FALSE)
npsas2.CA <- read.csv("Powerstats ozhexe.csv", skip=73, nrow=7, header=FALSE)
...
npsas2$Group <- factor(npsas2$Group, levels = c("White", "Latino/a", "Black", "Asian"))

npsas2$`Average loans` <- gsub(pattern="!", x=npsas2$`Average loans`, replacement="")
npsas2$`Average loans` <- gsub(pattern=",", x=npsas2$`Average loans`, replacement="")
npsas2$`Average loans` <- gsub(pattern=" ", x=npsas2$`Average loans`, replacement="")
...
npsas2P.US <- read.csv("Powerstats lkxsci.csv", skip=14, nrow=7, header=FALSE) %>% select(V1, V3)
npsas2P.CA <- read.csv("Powerstats lkxsci.csv", skip=78, nrow=7, header=FALSE) %>% select(V1, V3)
...
npsas2P$Group <- factor(npsas2P$Group, levels = c("White", "Latino/a", "Black", "Asian"))

npsas2P$`Share Borrowing Loans` <- gsub(pattern="!", x=npsas2P$`Share Borrowing Loans`, replacement="")
npsas2P$`Share Borrowing Loans` <- gsub(pattern=",", x=npsas2P$`Share Borrowing Loans`, replacement="")
npsas2P$`Share Borrowing Loans` <- gsub(pattern=" ", x=npsas2P$`Share Borrowing Loans`, replacement="")
...
```{r npsas2.1}
npsas2.1 <- read.csv("Powerstats odiegv.csv", skip=12, nrow=14, header=FALSE)
...
npsas2.1$Group <- factor(npsas2.1$Group, levels = c("White", "Latino/a", "Black", "Asian", "2+ races"))

npsas2.1$`Average loans` <- gsub(pattern="!", x=npsas2.1$`Average loans`, replacement="")
npsas2.1$`Average loans` <- gsub(pattern=",", x=npsas2.1$`Average loans`, replacement="")
npsas2.1$`Average loans` <- gsub(pattern=" ", x=npsas2.1$`Average loans`, replacement="")
...
npsas2.1P <- read.csv("Powerstats jcbvqb.csv", skip=14, nrow=14, header=FALSE) %>% select(V1, V3)
...
npsas2.1P$Group <- factor(npsas2.1P$Group, levels = c("White", "Latino/a", "Black", "Asian", "2+ races"))

npsas2.1P$`Share Borrowing Loans` <- gsub(pattern="!", x=npsas2.1P$`Share Borrowing Loans`, replacement="")
npsas2.1P$`Share Borrowing Loans` <- gsub(pattern=",", x=npsas2.1P$`Share Borrowing Loans`, replacement="")
npsas2.1P$`Share Borrowing Loans` <- gsub(pattern=" ", x=npsas2.1P$`Share Borrowing Loans`, replacement="")
...
**Data source:** National Postsecondary Student Aid Study, Undergraduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval number *ozhexe* for "Figure 3: Race" and *odiegv* for "Figure 3: Race and Gender."

**Timeframe reflected:** Figures in this graph reflect students enrolled in the 2015-16 academic year. 

**Notes:** Due to limitations of NPSAS:16, the data in Figure 3 only apply to in-state students. For "Figure 3: Race and Gender," only the U.S. is used due to insufficient sample size for many subgroups in California. The averages here do not include those who did not borrow federal loans. These measures include all federal student loans and do not include any private or institutional loans. 

***

#### {-}

Differences by group within California are also concerning. More than 75 percent of Black California graduate students and 62 percent of Latino/a California graduate students must borrow student loans, compared to 42 percent of white California graduate students. Although the share who borrow is roughly the same in California as in the U.S. overall, Black students are the exception, borrowing loans at a rate that is 13 percentage points higher. 

At the graduate level in California, Asian students borrow the most, exceeding $30,000 per year, which is slightly above the national average for Asian graduate students. However, Asian graduate students borrow the least frequently of these four groups, in California and in the U.S. overall. (Recall that this is limited to in-state students, so international enrollment is not a factor.)
...
npsas3$Group <- factor(npsas3$Group, levels = c("White", "Latino/a", "Black", "Asian", "2+ races"))
...
npsas3$`For Tooltip` <- paste(
  ...
  "Group: ", npsas3$Group, '\n',
  "Level: ", npsas3$Level, '\n',
  "Average annual federal loans: ", dollar(npsas3$`Total federal loans`, accuracy=1),
  sep=""
)

g4A <- ggplot(data=npsas3, mapping=aes(x=Group, y=`Total federal loans`, fill=Group, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.6) + labs(title="Average annual graduate loans by program level and group: U.S. only", y="Average Loans") + facet_grid(. ~ Level) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g4A, width=800, height=350, tooltip="text")
```

```{r percentages.npsas3}
npsas3P.US.masters <- read.csv("PowerStats lmjixp.csv", skip=78, nrow=7, header=FALSE) %>% select(V1, V3)
npsas3P.US.professional <- read.csv("PowerStats lmjixp.csv", skip=270, nrow=7, header=FALSE) %>% select(V1, V3)
...
npsas3P$Group <- factor(npsas3P$Group, levels = c("White", "Latino/a", "Black", "Asian", "2+ races"))

npsas3P$`Share Borrowing Loans` <- gsub(pattern="!", x=npsas3P$`Share Borrowing Loans`, replacement="")
npsas3P$`Share Borrowing Loans` <- gsub(pattern=",", x=npsas3P$`Share Borrowing Loans`, replacement="")
npsas3P$`Share Borrowing Loans` <- gsub(pattern=" ", x=npsas3P$`Share Borrowing Loans`, replacement="")
...
```{r npsas3.1}
npsas3.1.US.masters <- read.csv("PowerStats xqcxry.csv", skip=108, nrow=14, header=FALSE)
npsas3.1.US.professional <- read.csv("PowerStats xqcxry.csv", skip=396, nrow=14, header=FALSE)

names(npsas3.1.US.masters) <- c("Group", "Total federal loans")
names(npsas3.1.US.professional) <- c("Group", "Total federal loans")
...
npsas3.1 <- npsas3.1 %>% filter(Group %in% c("White",  "Latino/a", "Black", "Asian"))
npsas3.1$Group <- as.factor(npsas3.1$Group)
npsas3.1$Group <- factor(npsas3.1$Group, levels = c("White", "Latino/a", "Black", "Asian", "2+ races"))

npsas3.1$`Total federal loans` <- gsub(pattern="!", x=npsas3.1$`Total federal loans`, replacement="")
npsas3.1$`Total federal loans` <- gsub(pattern=",", x=npsas3.1$`Total federal loans`, replacement="")
npsas3.1$`Total federal loans` <- gsub(pattern=" ", x=npsas3.1$`Total federal loans`, replacement="")
npsas3.1$`Total federal loans` <- as.numeric(npsas3.1$`Total federal loans`)
```
...
npsas3.1$`For Tooltip` <- paste(
  ...
  "Average annual federal loans: ", dollar(npsas3.1$`Total federal loans`, accuracy=1),
  sep=""
)

g4B <- ggplot(data=npsas3.1, mapping=aes(x=Group, y=`Total federal loans`, fill=Group, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.6) + labs(title="Average annual graduate loans by program level, group and gender: U.S. only", y="Average Loans") + facet_grid(Gender ~ Level) + scale_y_continuous(labels=scales::dollar_format()) + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g4B, width=800, height=450, tooltip="text")
```

```{r percentages.npsas3.1}
npsas3.1P.US.masters <- read.csv("PowerStats ilzmdk.csv", skip=113, nrow=14, header=FALSE) %>% select(V1, V3)
npsas3.1P.US.professional <- read.csv("PowerStats ilzmdk.csv", skip=410, nrow=14, header=FALSE) %>% select(V1, V3)
...
npsas3.1P <- npsas3.1P %>% filter(Group %in% c("White",  "Latino/a", "Black", "Asian", "2+ races"))
npsas3.1P$Group <- as.factor(npsas3.1P$Group)
npsas3.1P$Group <- factor(npsas3.1P$Group, levels = c("White", "Latino/a", "Black", "Asian", "2+ races"))

npsas3.1P$`Share Borrowing Loans` <- gsub(pattern="!", x=npsas3.1P$`Share Borrowing Loans`, replacement="")
npsas3.1P$`Share Borrowing Loans` <- gsub(pattern=",", x=npsas3.1P$`Share Borrowing Loans`, replacement="")
npsas3.1P$`Share Borrowing Loans` <- gsub(pattern=" ", x=npsas3.1P$`Share Borrowing Loans`, replacement="")
...
**Data source:** National Postsecondary Student Aid Study, Graduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval number *enucsq* for "Figure 4: Race" and table retrieval number *xqcxry* for "Figure 4: Race and Gender."

**Timeframe reflected:** Figures in this graph reflect students enrolled in the 2015-16 academic year. 

**Notes:** The averages here do not include those who did not borrow federal loans. These measures include all federal student loans and do not include any private or institutional loans. In contrast to other charts in this section, Figure 4 is not limited to in-state students, since it does not have California-only statistics. 
...
Figure 5 returns to the undergraduate level and considers how much debt undergraduate students earn by the time they are about to earn their bachelor's degree. Specifically, I examine here undergraduate students who are in their 4th or 5th undergraduate year in terms of credit accumulation.  

#### {.tabset}
##### Figure 5: Average Undergraduate Loans at Graduation by Race
```{r npsas4, warning=FALSE}
npsas4.US <- read.csv("PowerStats dxwsoi.csv", skip=12, nrow=7, header=FALSE)
npsas4.CA <- read.csv("PowerStats dxwsoi.csv", skip=73, nrow=7, header=FALSE)

names(npsas4.US) <- c("Group", "Average cumulative federal loans")
names(npsas4.CA) <- c("Group", "Average cumulative federal loans")
...
npsas4$`Average cumulative federal loans` <- as.numeric(npsas4$`Average cumulative federal loans`)
...
npsas4$`For Tooltip` <- paste(
  "State: ", npsas4$State, '\n',
  "Group: ", npsas4$Group, '\n',
  "Average cumulative federal loans: ", dollar(npsas4$`Average cumulative federal loans`, accuracy=1),
  sep=""
)

g5A <- ggplot(data=npsas4, mapping=aes(x=State, y=`Average cumulative federal loans`, fill=State, text=`For Tooltip`)) + geom_bar(stat="identity", position="dodge", width=0.6) + facet_grid(. ~ Group) + scale_y_continuous(label=scales::dollar) + labs(x="State", title="Graduating undergraduates' average cumulative loans by group", y="Average cumulative loans") + scale_fill_manual(values = stateCols) + theme(legend.position="none", text=element_text(family="Optima"))

ggplotly(g5A, width=800, height=350, tooltip="text")
```

```{r percentages.npsas4}
npsas4P.US <- read.csv("PowerStats dyclij.csv", skip=14, nrow=7, header=FALSE) %>% select(V1, V3)
npsas4P.CA <- read.csv("PowerStats dyclij.csv", skip=78, nrow=7, header=FALSE) %>% select(V1, V3)
...
npsas4P$Group <- factor(npsas4P$Group, levels = c("White", "Latino/a", "Black", "Asian", "Pacific Islander", "2+ races"))
```
...
npsas4.1.US <- read.csv("PowerStats gkhfjw.csv", skip=12, nrow=14, header=FALSE)
npsas4.1.CA <- read.csv("PowerStats gkhfjw.csv", skip=108, nrow=14, header=FALSE)
...
npsas4.1$Group <- factor(npsas4.1$Group, levels = c("White", "Latino/a", "Black", "Asian", "2+ races"))
...
npsas4.1P$Group <- factor(npsas4.1P$Group, levels = c("White", "Latino/a", "Black", "Asian", "2+ races"))
...
**Data source:** National Postsecondary Student Aid Study, Undergraduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval numbers *dxwsoi* for "Figure 5: Race" and table retrieval number *gkhfjw* for "Figure 5: Race and Gender (U.S. only)".

**Timeframe reflected:** Figures in this graph reflect students enrolled in the 2015-16 academic year. 

**Notes:** Due to limitations of NPSAS:16, the data in Figure 5 only apply to in-state students. The averages here do not include those with no cumulative federal loans. These measures include all federal student loans (including Parent PLUS) and do not include any private or institutional loans. 

***

#### {-}

California students fare better here than students nationwide, in terms of the share who borrow and the cumulative loans of those who do. One exception is that California Pacific Islander borrowers average more than those nationwide, almost \$34,000 by graduation. 

Nearly 85 percent of Black California undergraduates borrow student loans by graduation, which is slightly lower than the national rate for Black undergraduates but still exceeds all other groups by a concerning margin. Latino/a California undergraduates have the second-highest frequency at 67 percent. Notably, all groups are above 50 percent in California as in the U.S. overall. 

The trend of pronounced borrowing among women, and especially Black women, extends here as well. Nine in ten Black California undergraduate women borrow by the time they graduate, on par with the national rate but well above other groups. 

NPSAS allows us to disaggregate by ethnicity (to a limited extent) for two racial groups, Hispanic and Asian. No individual Asian ethnic group stands out from the rest along these measures, although Indian, Korean, Filipino, and Mixed Asian California undergraduates all average more loans than other groups and exceed the corresponding national averages. Fewer distinct groups are available in the Hispanic variable; Puerto Rican California undergraduates show the highest average loans. 
...
npsas5P$`Share Borrowing Loans` <- gsub(pattern="!", x=npsas5P$`Share Borrowing Loans`, replacement="")
npsas5P$`Share Borrowing Loans` <- gsub(pattern=",", x=npsas5P$`Share Borrowing Loans`, replacement="")
npsas5P$`Share Borrowing Loans` <- gsub(pattern=" ", x=npsas5P$`Share Borrowing Loans`, replacement="")
...
**Data source:** National Postsecondary Student Aid Study, Undergraduate. Datalab interface available [here](https://nces.ed.gov/datalab/) using table retrieval number *famipl.*

**Timeframe reflected:** Figures in this graph reflect students enrolled in the 2015-16 academic year. 

**Notes:** No other racial groups besides Hispanic and Asian have breakouts by ethnicity in NPSAS. Due to limitations of NPSAS:16, the data in Figure 6 only apply to in-state students. The averages here do not include those who did not borrow federal loans. These measures include all federal student loans (including Parent PLUS) and do not include any private or institutional loans. 
...
**Notes:** Only 73 institutions have at least 500 borrowers with outstanding Grad PLUS debt. 
...
**Notes:** I applied a threshold of minimum 200 borrowers per institution in the ten-year repayment cohort. This does not include those in forbearance or deferment.
...
With Figure 36, I am interested in whether there is any relationship between the racial demographics of those who complete California programs and their earnings. Starting with the same aggregated program-level earnings data from College Scorecard as I used for Figure 35, I link in data on the demographics of program completers from IPEDS. The first tab in Figure 36 focuses on Black students' share of completers, and it compares the rank of Black students' share of completers by ranked earnings. 

The key question when interpreting this data is whether a high/low rank in one variable seems to correlate with a high/low rank in the other variable. While this is only a cursory analysis, no such patterns emerge. There is perhaps a slight correlation in bachelor's degree programs, where a lower share of Black enrollment is associated with a higher earnings average; however, this seems to be a modest correlation at the most. The same holds true for the tab on the Hispanic share of completers. 

### {.tabset}
```{r EarningsDemographics, warning=FALSE}
Q26.1 <- c2yr %>% select(
  `UNITID`, 
  `OPEID`,
  `STABBR`,
...
Q26.2 <- Q26.2 %>% filter(EARN_MDN_HI_2YR != "PrivacySuppressed") 
Q26.2 <- Q26.2 %>% filter(EARN_COUNT_WNE_HI_2YR != "PrivacySuppressed")
...
Q26.2$EARN_MDN_HI_2YR <- as.numeric(Q26.2$EARN_MDN_HI_2YR)
Q26.2$EARN_COUNT_WNE_HI_2YR <- as.numeric(Q26.2$EARN_COUNT_WNE_HI_2YR)

cipNames.26 <- Q26.2 %>% select(CIPCODE, CIPDESC) %>% filter(duplicated(CIPCODE)==FALSE)

Q26.3 <- aggregate(data=Q26.2, EARN_COUNT_WNE_HI_2YR ~ CIPCODE + CREDDESC, FUN=sum)
names(Q26.3) <- c("CIPCODE", "CREDDESC", "Total earner count")
Q26.2 <- left_join(x=Q26.2, y=Q26.3, by=c("CIPCODE", "CREDDESC"))

Q26.2$`Share of all earners` <- Q26.2$EARN_COUNT_WNE_HI_2YR / Q26.2$`Total earner count`
Q26.2$`Weighted earnings median` <- Q26.2$`Share of all earners` * Q26.2$EARN_MDN_HI_2YR
Q26.2 <- aggregate(data=Q26.2, `Weighted earnings median` ~ CIPCODE + CREDDESC, FUN=sum)
Q26.2 <- left_join(x=Q26.2, y=cipNames.26, by="CIPCODE")
...
Q26.Bplot$`Rank of BKAA share` <- rank(-(Q26.Bplot$`Share who are BKAA`), ties.method="average")
Q26.Hplot$`Rank of HISP share` <- rank(-(Q26.Hplot$`Share who are HISP`), ties.method="average")
```
#### Figure 36: California programs by average earnings and Black/African-American enrollment
```{r figure36A}
Q26.Bplot$`For Tooltip` <- paste(
  "Program name: ", Q26.Bplot$CIPDESC, '\n',
  "Credential level: ", Q26.Bplot$CREDDESC, '\n',
  "Black share of completers: ", percent(Q26.Bplot$`Share who are BKAA`, accuracy=0.1), '\n',
  "Rank of Black share of completers: ", as.integer(Q26.Bplot$`Rank of BKAA share`), '\n',
  "Estimated average earnings: ", dollar(Q26.Bplot$`Weighted earnings median`, accuracy=1), '\n',
  "Rank of earnings among completers: ", as.integer(Q26.Bplot$`Rank of weighted earnings`), 
  sep=""
)

g36A <- ggplot(data=Q26.Bplot, mapping=aes(x=`Rank of BKAA share`, y=`Rank of weighted earnings`, fill=CREDDESC, text=`For Tooltip`)) + geom_point(stat="identity") + facet_wrap(`CREDDESC` ~ .) + labs(x="Rank of Black share of completers (1=Highest share)", y="Rank of earnings among completers (1=Highest earnings)") + theme(text=element_text(family="Optima"), legend.position="none") + scale_fill_brewer(palette="Set2") 

ggplotly(g36A, width=800, height=600, tooltip="text")
```
#### Figure 36: California programs by average earnings and Hispanic/Latino enrollment
```{r figure36B}
Q26.Hplot$`For Tooltip` <- paste(
  "Program name: ", Q26.Hplot$CIPDESC, '\n',
  "Credential level: ", Q26.Hplot$CREDDESC, '\n',
  "Hispanic share of completers: ", percent(Q26.Hplot$`Share who are HISP`, accuracy=0.1), '\n',
  "Rank of Hispanic share of completers: ", as.integer(Q26.Hplot$`Rank of HISP share`), '\n',
  "Estimated average earnings: ", dollar(Q26.Hplot$`Weighted earnings median`, accuracy=1), '\n',
  "Rank of earnings among completers: ", as.integer(Q26.Hplot$`Rank of weighted earnings`), 
  sep=""
)

g36B <- ggplot(data=Q26.Hplot, mapping=aes(x=`Rank of HISP share`, y=`Rank of weighted earnings`, fill=CREDDESC, text=`For Tooltip`)) + geom_point(stat="identity") + facet_wrap(`CREDDESC` ~ .) + labs(x="Rank of Hispanic share of completers (1=Highest share)", y="Rank of earnings among completers (1=Highest earnings)") + theme(text=element_text(family="Optima"), legend.position="none") + scale_fill_brewer(palette="Set2") 

ggplotly(g36B, width=800, height=600, tooltip="text")
```
#### Data Source and Notes

**Data source:** These figures were calculated using data from two sources. First is the College Scorecard field-of-study dataset, accessed November 2022, available through the U.S. Department of Education [here](https://collegescorecard.ed.gov/data/). Second is the IPEDS Completion Surveys, accessed November 2022, available through NCES [here](https://nces.ed.gov/ipeds/use-the-data). 

**Timeframe reflected:** Ranked earnings values are author's calculations using College Scorecard data reflecting the calendar year 2017 or 2018 earnings of students who received federal student aid and graduated from their program in academic year 2014-15 or 2015-16. Ranked shares of completers by race are author's calculations using data from the IPEDS Completion Surveys reflecting 2019-20 and 2020-21. 
...
Figure 37 operates the same as Figure 36, except its focus is average debt instead of average earnings. Once again, there does not appear to be a correlation that provides us useful insights. 
...
cipNames.27 <- Q27 %>% select(CIPCODE, CIPDESC) %>% filter(duplicated(CIPCODE)==FALSE)
...
Q27 <- left_join(x=Q27, y=cipNames.27, by="CIPCODE")
...
Q27.Bplot$`Rank of BKAA share` <- rank(-(Q27.Bplot$`Share who are BKAA`), ties.method="average")
Q27.Hplot$`Rank of HISP share` <- rank(-(Q27.Hplot$`Share who are HISP`), ties.method="average")
```
#### Figure 37: California programs by average debt and Black/African-American enrollment
```{r figure37A}
Q27.Bplot$`For Tooltip` <- paste(
  "Program name: ", Q27.Bplot$CIPDESC, '\n',
  "Credential level: ", Q27.Bplot$CREDDESC, '\n',
  "Black share of completers: ", percent(Q27.Bplot$`Share who are BKAA`, accuracy=0.1), '\n',
  "Rank of Black share of completers: ", as.integer(Q27.Bplot$`Rank of BKAA share`), '\n',
  "Estimated average debt: ", dollar(Q27.Bplot$`Weighted debt median`, accuracy=1), '\n',
  "Rank of debt among completers: ", as.integer(Q27.Bplot$`Rank of weighted debt`), 
  sep=""
)

g37A <- ggplot(data=Q27.Bplot, mapping=aes(x=`Rank of BKAA share`, y=`Rank of weighted debt`, fill=CREDDESC, text=`For Tooltip`)) + geom_point(stat="identity") + facet_wrap(`CREDDESC` ~ .) + labs(x="Rank of Black share of completers (1=Highest share)", y="Rank of debt among completers (1=Highest debt)") + theme(text=element_text(family="Optima"), legend.position="none") + scale_fill_brewer(palette="Set2") 

ggplotly(g37A, width=800, height=600, tooltip="text")
```
#### Figure 37: California programs by average debt and Hispanic/Latino enrollment
```{r figure37B}
Q27.Hplot$`For Tooltip` <- paste(
  "Program name: ", Q27.Hplot$CIPDESC, '\n',
  "Credential level: ", Q27.Hplot$CREDDESC, '\n',
  "Hispanic share of completers: ", percent(Q27.Hplot$`Share who are HISP`, accuracy=0.1), '\n',
  "Rank of Hispanic share of completers: ", as.integer(Q27.Hplot$`Rank of HISP share`), '\n',
  "Estimated average debt: ", dollar(Q27.Hplot$`Weighted debt median`, accuracy=1), '\n',
  "Rank of debt among completers: ", as.integer(Q27.Hplot$`Rank of weighted debt`), 
  sep=""
)

g36B <- ggplot(data=Q27.Hplot, mapping=aes(x=`Rank of HISP share`, y=`Rank of weighted debt`, fill=CREDDESC, text=`For Tooltip`)) + geom_point(stat="identity") + facet_wrap(`CREDDESC` ~ .) + labs(x="Rank of Hispanic share of completers (1=Highest share)", y="Rank of debt among completers (1=Highest debt)") + theme(text=element_text(family="Optima"), legend.position="none") + scale_fill_brewer(palette="Set2") 
...
**Timeframe reflected:**  Ranked debt values are author's calculations using College Scorecard data reflecting borrowers who entered repayment in award year 2017-18 or 2018-19. Ranked shares of completers by race are author's calculations using data from the IPEDS Completion Surveys reflecting 2019-20 and 2020-21. 
...
The final analyses of this document will use College Scorecard data to show how federal student loan repayment in California compares to the rest of the U.S. 
...
We can calculate a cohort's "DBRR ratio" by dividing its original total balance by its current total balance. When the ratio is over 1, the borrowers owe more than they originally borrowed, presumably due to the accumulation of interest outpacing the borrowers' payments. When the ratio is under 1, the borrowers are making progress on repayment. 

To be clear, DBRR does not track the same cohort of borrowers and report their loan balances at 1, 5, 10, and 20 years. Rather, each of these cohorts is a separate group of borrowers. It is also important to note the following: 

* These data reflect pre-pandemic balances. 
* Not all colleges and universities have DBRR data in the College Scorecard. In Figures 38 and 39, you can hover over the points in the chart to see the count of institution represented in each cohort. For-profit colleges are especially under-represented, with only one California for-profit included. Interpret the charts on for-profits in Figures 38 and 39 with caution. 

DBRR data are reported at the institution level and disaggregated by loan type (Stafford, Parent PLUS, and Grad PLUS). I aggregated the DBRR values across all institutions with sufficient data and compared institutions in California to institutions in the rest of the U.S., with breakouts by institutional control. 

Figure 38 shows the DBRR ratios for the available cohorts of borrowers in repayment. When the blue line (California) is higher than the yellow line (Rest of U.S.), it means that borrowers from California institutions owe a larger share of their original balance than do those from institutions across the rest of the U.S. 

In all three tabs, we see that loan borrowers from California public institutions tend to have better (i.e. lower) DBRR ratios than loan borrowers from public institutions in the rest of the country, regardless of loan type. In the DBRR ratios for borrowers who took out loans for a nonprofit institution, California institutions roughly match institutions nationwide.  

Overall, California performs well along DBRR metrics compared to the rest of the U.S. However, this same data can be used to track cohorts' average loan balances, which reveals some concerning trends for California. I derive the average loan balance by dividing current balances by borrower count. Note that a borrower who has fully paid off their loan is still counted in the average measure.  
...
g38B <- ggplot(data=Q28.P, mapping=aes(x=Year, y=Ratio, group=State, text=`For Tooltip`)) + geom_point(aes(color=State)) + geom_line(aes(color=State)) + facet_wrap(Control ~ ., nrow=2) + scale_y_continuous(limits=c(0, 1.4)) + labs(x="Years into repayment", y="Ratio of current balance to original balance") + theme(text=element_text(family="Optima"), legend.position="right") + scale_color_manual(values = stateCols)

ggplotly(g38B, width=800, height=600, tooltip="text")
```
#### Figure 38: Grad PLUS
```{r figure38C}
...
g38C <- ggplot(data=Q28.G, mapping=aes(x=Year, y=Ratio, group=State, text=`For Tooltip`)) + geom_point(aes(color=State)) + geom_line(aes(color=State)) + facet_wrap(Control ~ ., nrow=2) + scale_y_continuous(limits=c(0, 1.4)) + labs(x="Years into repayment", y="Ratio of current balance to original balance") + theme(text=element_text(family="Optima"), legend.position="right") + scale_color_manual(values = stateCols)

ggplotly(g38C, width=800, height=600, tooltip="text")
```
#### Data Source and Notes
...
Figure 39 shows the average current loan amount by institutional control and loan type, comparing California institutions to those in the rest of the U.S. Average Stafford loan amounts are lower among borrowers from California institutions than those from institutions across the rest of the U.S., driven by lower average loans at California's public institutions. 
